...
class Logger extends commons_1.Utility {
     ...
     */
    bufferLogThreshold = constants_js_1.LogLevelThreshold.DEBUG;
    /**
     ...
     */
    #buffer = new logBuffer_js_1.CircularMap({
        ...
        for (const value of [constructorValue, customConfigValue, envVarsValue]) {
            if (this.isValidSampleRate(value)) {
                ...
                if (this.logLevel > constants_js_1.LogLevelThreshold.DEBUG &&
                    value &&
                    (0, node_crypto_1.randomInt)(0, 100) / 100 <= value) {
    ...
    flushBuffer() {
        ...
        for (const item of buffer) {
            const consoleMethod = item.logLevel === constants_js_1.LogLevelThreshold.CRITICAL
                ? 'error'
                : this.getLogLevelNameFromNumber(item.logLevel).toLowerCase();
